...
void IntegerToken::outputTo(std::ostream& ostream) const {
    ostream << value;
}

long IntegerToken::getValue() const {
    return value;
}

void IntegerToken::setValue(long newValue) {
    value = newValue;
}

bool IntegerToken::isZero() const {
    return value == 0;
}

bool IntegerToken::isUnity() const {
    return value == 1;
}
